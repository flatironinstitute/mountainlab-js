...
function prv_locate(prv_fname,callback) {
	...
	var orig_path=obj.original_path||'';
	if (!sha1) {
		callback('original_checksum field not found in prv file: '+prv_fname);
		return;
	}
	var doc0=sumit.find_doc_by_sha1(sha1,prv_search_paths);
	if (doc0) {
		callback('',doc0.path);
		return;
	}
...
sumit.find_doc_by_sha1=function(sha1,valid_prv_search_paths) {
	var CC=db_utils.open_collection('sumit');
	var docs=CC.find({sha1:sha1});
...
sumit.find_doc_by_path=function(path) {
	var CC=db_utils.open_collection('sumit');
	var docs=CC.find({path:path});
	if (docs.length===0) return null;
	for (var i in docs) {
		var doc0=docs[i];
		if (sumit.file_matches_doc(doc0.path,doc0)) {
...
sumit.compute_file_sha1=function(path,callback) {
	...
	var doc0=sumit.find_doc_by_path(path);
	if (doc0) {
		...
		if (stat0) {
			if (stat0.isFile()) {
				if (stat0.size==size) { //candidate
					var doc0=sumit.find_doc_by_path(fname);
					if (doc0) {
						if (doc0.sha1==sha1) {
							callback('',fname)
							return;
